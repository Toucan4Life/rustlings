...
impl From<&str> for Person {
    fn from(s: &str) -> Self {
        ...
        if t.len() != 2 {return Default::default();}
        if  t[0].is_empty(){return Default::default();}
        if let Ok(parsed_age) =  t[1].parse::<u8>(){
           return Person { name:  t[0].to_string(), age:parsed_age  };
        }
